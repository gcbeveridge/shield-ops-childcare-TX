...
- **Smart Alerts System (Phase 2D):**
    - **Purpose:** Intelligent alerts system that powers notifications and priority tracking with audit trail.
    - **Database Tables:** alert_rules (configurations), alerts (active notifications), alert_history (audit trail).
    - **Alert Types:** cert_expired, cert_expiring, missing_spot_check, missing_documents, ratio_violation.
    - **Severity Levels:** critical (red), warning (yellow), info (blue).
    - **Notification Bell:** Located in sidebar header, shows unread count badge.
    - **Notification Dropdown:** Click bell to see recent alerts, mark as read, navigate to action.
    - **Auto-Generation:** POST /api/facilities/:id/alerts/generate scans staff certifications, spot-checks, documents.
    - **Acknowledgment:** Clicking an alert marks it as acknowledged with user name and timestamp.
    - **Audit Trail:** All actions (created, acknowledged, resolved) logged to alert_history table.
    - **API Endpoints:** GET/POST /api/facilities/:id/alerts, GET /api/facilities/:id/alerts/count, PATCH /api/facilities/:id/alerts/:alertId/acknowledge, PATCH /api/facilities/:id/alerts/:alertId/resolve.
