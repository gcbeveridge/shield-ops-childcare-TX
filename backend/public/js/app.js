...
async function loadNotifications() {
    try {
        ...
        await apiRequest(`/facilities/${facilityId}/alerts/generate`, {
            method: 'POST'
        });

        // Fetch alerts
        const response = await apiRequest(`/facilities/${facilityId}/alerts`);
...
function displayNotifications(alerts) {
    ...
    if (!alerts || alerts.length === 0) {
        container.innerHTML = `
            <div class="notification-empty">
                ...
                <div style="font-size: 0.8125rem;">No new notifications</div>
            </div>
        `;
        return;
    }
    ...
    container.innerHTML = recentAlerts.map(alert => `
        <div class="notification-item ${alert.acknowledged ? '' : 'unread'}" 
             onclick="handleNotificationClick('${alert.id}', '${alert.action_url || '/dashboard'}')">
            <div class="notification-icon">${getSeverityIcon(alert.severity)}</div>
            <div class="notification-content">
                <div class="notification-title">${escapeHtml(alert.title)}</div>
    ...
    try {
        ...
        for (const alert of alerts) {
            if (!alert.acknowledged) {
                await apiRequest(`/facilities/${facilityId}/alerts/${alert.id}/acknowledge`, {
                    method: 'PATCH',
                    body: JSON.stringify({ acknowledged_by_name: userName })
                });
            }
...
// Load notification badge on dashboard load
async function loadNotificationBadge() {
    try {
        const facilityId = AppState.facility?.id;
        if (!facilityId) return;
        
        const response = await apiRequest(`/facilities/${facilityId}/alerts/count`);
